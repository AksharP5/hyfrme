...
All 39 free visual components have fixtures in `catalog/snapcn-fixtures.json`.
The source inventory is `catalog/snapcn-upstream.json`.

The 36 entries in the merged preview registry retain its exact control defaults,
...
## Published coverage, September 9, 2026

The [published registry](https://snapcn.dev/r/registry.json) lists 39 free visuals,
14 Pro visuals, and one shared runtime item. The public repository at
[`099cb549d472bc63dd7ad8ca0f28ff0f39b71a51`](https://github.com/snapcndev/snapcn/commit/099cb549d472bc63dd7ad8ca0f28ff0f39b71a51)
has unchanged free component source and documentation compared with `bc5b59f`.
Existing fixture pins and render evidence remain unchanged.
...
The [upstream registry route](https://github.com/snapcndev/snapcn/blob/099cb549d472bc63dd7ad8ca0f28ff0f39b71a51/app/r/%5Bfile%5D/route.ts)
...
endpoint results, and source URLs without changing the pinned port inventory.
...
ships with that block. The two additions reuse frozen Inter fonts and introduce
no new media or shader dependency.

The 10 font families use unmodified Latin binaries selected from
`@remotion/google-fonts@4.0.473` metadata. Seventeen unique binaries cover all
...
The 87 frozen media files include all 24 follower avatars and 16 Orbit Gallery
...
The audit passes for all 39 generated blocks and 108 frozen font/media assets,
including the original and derived videos. It checks the derived videos' actual
transfer, matrix, and primaries metadata with `ffprobe` to prevent browser color
regressions.
All 565 generated controls preserve the pinned source defaults, labels, options,
